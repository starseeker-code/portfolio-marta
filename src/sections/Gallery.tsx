  ...
  return (
    <section id="gallery" className="py-24 bg-white">
      <div className="container mx-auto px-6">
        ...
        {/* Filter tags */}
        <RevealOnScroll delay={100} className="flex flex-wrap justify-center gap-3 mb-12">
          {tags.map((tag) => (
            <button
              key={tag}
              ...
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                activeTag === tag
                  ? 'bg-barbie-gradient text-white shadow-md scale-105'
        ...
        {/* Grid — each card reveals with stagger */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
          {filtered.map((item, i) => (
            <RevealOnScroll key={item.id} delay={i * 70}>
              <div className="group relative aspect-square rounded-2xl overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-shadow duration-300">
                {/* Shimmer placeholder */}
                ...
                <div className="absolute inset-0 bg-barbie-gradient opacity-0 group-hover:opacity-85 transition-opacity duration-300 flex items-center justify-center">
                  <div className="text-white text-center translate-y-4 group-hover:translate-y-0 transition-transform duration-300 px-4">
        ...
        {/* CTA */}
        <RevealOnScroll delay={200} className="text-center mt-14">
